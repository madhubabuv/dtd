...
class ViTExtractor:
    ...
    def __init__(self, model_type: str = 'dinov2_vitb14', stride: int = 14, model: nn.Module = None, device: str = 'cuda'):
        """
    ...
    def create_model(model_type: str) -> nn.Module:
        ...
        model = torch.hub.load(repo_or_dir="facebookresearch/dinov2", model=model_type)
    ...
    def _fix_pos_enc(patch_size: int, stride_hw: Tuple[int, int]):
        ...
        """
        def interpolate_pos_encoding(self,x, height ,width) -> torch.Tensor:

            npatch = x.shape[1] - 1
            
            N = self.pos_embed.shape[1] - 1
            if npatch == N and width == height:
                return self.pos_embed
            class_pos_embed = self.pos_embed[:, 0]
            patch_pos_embed = self.pos_embed[:, 1:]
            dim = x.shape[-1]
            # compute number of tokens taking stride into account
            w0 = 1 + (width - patch_size[0]) // stride_hw[1]
            h0 = 1 + (height - patch_size[0]) // stride_hw[0]
            assert (w0 * h0 == npatch), f"""got wrong grid size for {height}x{width} with patch_size {patch_size} and 
    ...
    def _get_hook(self, facet: str, idx = None):
        ...
        if facet in ['attn', 'token']:
            def _hook(model, input, output):
                ...
                if not idx is None:
                    coarse_height = 1 + (self.image_height - self.p[0]) // self.stride[0]
                    coarse_width = 1 + (self.image_width - self.p[0]) // self.stride[1]
                    cls_token = output[:,:1,:]
