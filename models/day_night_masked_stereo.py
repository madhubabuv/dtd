...
class UniMatch(nn.Module):
    ...
    def get_cross_mask(self, feat0:torch.tensor, feat1:torch.tensor, thr:float = 0.2):

        norm_feat0 = torch.nn.functional.normalize(feat0, dim=1)
        norm_feat1 = torch.nn.functional.normalize(feat1, dim=1)

        shape = norm_feat0.shape
        re_feat0 = norm_feat0.permute(0, 2, 3, 1).contiguous()
        re_feat0 = re_feat0.view(re_feat0.shape[0], -1, re_feat0.shape[3] * re_feat0.shape[2])
        re_feat1 = norm_feat1.permute(0, 2, 3, 1).contiguous()
        re_feat1 = re_feat1.view(re_feat1.shape[0], -1, re_feat1.shape[3] * re_feat1.shape[2])

        similarity = torch.bmm(re_feat0, re_feat1.permute(0, 2, 1).contiguous())
        nearest_nn_inds = torch.topk(similarity, k=2, dim=-1, largest=True)[1][:, :, 1]
        nearest_nn = torch.gather(re_feat1, dim=1, index=nearest_nn_inds.unsqueeze(-1).expand(-1, -1, re_feat0.shape[-1]))
        distance = torch.norm(re_feat0 - nearest_nn, dim=-1)
        mask = torch.where(distance < thr, torch.ones_like(distance), torch.zeros_like(distance))
        mask = mask.view(shape[0],1, shape[2], shape[3])
    ...
    def get_mask(self, features:torch.tensor, thr:float = 0.2):
    ...
    def forward(self, img0, img1,
        ...
        for scale_idx in range(self.num_scales):
            ...
            #here we have both day and night features
            batch_size = feature0.shape[0]
            day_left_features = feature0[0:batch_size//2]
            night_left_features = feature0[batch_size//2:]

            cross_mask = self.get_cross_mask(night_left_features, day_left_features.detach(), thr[scale_idx])
            ...
            if scale_idx == 0: all_features.append(feature0)
